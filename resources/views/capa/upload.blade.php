    ...
    <center>
        ...
        @if($ekstensi == "pdf")
            ...
            <a class="btn btn-primary btn-xs" 
                href="{{ route('capa.show', $id) }}" target="_blank">
            <i class="fa fa-eye"></i></a>

            <button type="button" title="Upload" class="btn btn-warning btn-xs" data-toggle="modal"
            data-target="#uploadFile{{$id}}">
                <i class="fa fa-upload"></i>
            </button>

            <a class="btn btn-success btn-xs" 
                href="{{ route('capa.download', $id) }}">
            <i class="fa fa-download"></i></a>
        @else
            {{-- jika selain pdf --}}
            <button type="button" title="Lihat" class="btn btn-primary btn-xs" data-toggle="modal"
                data-target="#lihatFile{{$id}}"> <i class="fa fa-eye"></i>
            </button>
            ...
            <a class="btn btn-success btn-xs" title="Download" 
                href="{{ route('capa.download', $id) }}"> <i class="fa fa-download"></i>
            </a>
        @endif
    </center>
@else
    ...
    <div class="modal-dialog modal-lg" width="100%">
        <div class="modal-content">
            ...
            <div class="modal-header text-center">
                ...
                <h4 class="modal-title" id="myModalLabel">File / Dokumen CAPA<strong></strong></h4>
            </div>
            
            <div class="modal-body">
                <img src="{{ asset('storage/CAPA/'.$upload_file) }}" alt="{{$upload_file}}" width="100%">                
            </div>
...
<div class="modal fade" id="uploadFile{{$id}}" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-center">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Pilih File / Dokumen CAPA<strong></strong></h4>
            </div>
        <form action="{{ route('capa.upload', $id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <div class="modal-body">
                <center>
                <div class="form-group">
                    ...
                    @if($upload_file != null)
                        ...
                        @else                            
                            <img src="{{ asset('storage/CAPA/'.$upload_file)}}" style="width:45%; height:45%"><br>                            
                            <label for=""><strong>Nama File :</strong> {{ $upload_file}}</label><br>
                        @endif                    
                    @endif
