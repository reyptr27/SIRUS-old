<a href="{{ route('arsip.show', $id) }}" type="button" title="Lihat" class="btn btn-info btn-xs"><i class="fa fa-eye"></i> Show
</a>

<button type="button" title="Restore" class="btn btn-primary btn-xs" data-toggle="modal"
    data-target="#restoreArsip{{ $id }}">
...
<div class="modal fade" id="hapusArsip{{ $id }}" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        ...
        <form action="{{ route('trash.destroy', $id) }}" method="post">
            {{ csrf_field() }}
            {{ method_field('DELETE') }}
...
<div class="modal fade" id="restoreArsip{{ $id }}" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        <div class="modal-content">
        <div class="modal-header text-center">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="myModalLabel">Anda yakin ingin mengembalikan <strong>{{ $nama_arsip }}</strong> ke list data arsip ?</h4>
        </div>
        <form action="{{ route('trash.restore', $id) }}" method="post">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <div class="modal-footer">
                <center>
                    <button type="submit" class="btn btn-primary"><i class="fa fa-refresh"></i> RESTORE</button>
                    <button type="button" class="btn btn-danger" data-dismiss="modal"><i class="fa fa-close"></i>BATAL</button>
                </center>
            </div>
        </form>     
