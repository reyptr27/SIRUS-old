...
@if($status == 1)
    ...
    <div class="modal-dialog">
        ...
        <div class="modal-header text-center">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
            <h4 class="modal-title" id="myModalLabel">Anda yakin ingin menghapus rekomendasi pengiriman mesin <strong>{{ $nomor }}</strong> ?</h4>
        </div>
        <form action="{{ route('monitoringmesin.rekomendasi.hapus', $id) }}" method="post">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <div class="modal-body">
