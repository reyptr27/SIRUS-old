...
@if( $upload_file != null )
    <center>
        ...
        @if($ekstensi == "pdf")
            ...
            <a class="btn btn-primary btn-xs" href="{{ route('monitoringmesin.selesai.show', $id) }}" target="_blank">
            ...
            <a class="btn btn-danger btn-xs" href="{{ route('monitoringmesin.selesai.download', $id) }}">
        ...
        @else
            ...
            <a class="btn btn-danger btn-xs" title="Download" href="{{ route('monitoringmesin.selesai.download', $id) }}">
            <i class="fa fa-download"></i></a>
        @endif
    </center>
@else
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header text-center">
                ...
                <h4 class="modal-title" id="myModalLabel">Pilih File / Dokumen Berita Acara Serah Terima Mesin<strong></strong></h4>
            </div>
        <form action="{{ route('monitoringmesin.selesai.upload', $id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
            <div class="modal-body">
                <div class="form-group">
                    ...
                    @if($upload_file != null)
                        ...
                        @else                            
                            <img src="{{ asset('storage/MonitoringMesin/'.$upload_file)}}" style="width:45%; height:45%"><br>                            
                            <label for=""><strong>Nama File :</strong> {{ $upload_file}}</label><br>
                        @endif                    
                    @endif
...
<div class="modal fade" id="lihatFile{{ $id }}" role="dialog" aria-labelledby="myModalLabel">                               
    <div class="modal-dialog modal-lg" width="100%">
        <div class="modal-content">
            ...
            <div class="modal-header text-center">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Berita Acara Serah Terima Mesin<strong></strong></h4>
            </div>
