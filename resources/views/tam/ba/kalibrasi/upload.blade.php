...
<center>
    ...
    @if($ekstensi == "pdf")
        ...
        <a class="btn btn-primary btn-xs" href="{{ route('kalibrasi.show', $id) }}" target="_blank">
        ...
        <a class="btn btn-danger btn-xs" href="{{ route('kalibrasi.download', $id) }}">
    ...
    @else
        ...
        <a class="btn btn-danger btn-xs" title="Download" href="{{ route('kalibrasi.download', $id) }}">
        <i class="fa fa-download"></i></a>
    @endif
</center>
@else
...
<div class="modal fade" id="uploadBa{{$id}}" role="dialog" aria-labelledby="myModalLabel">
    <div class="modal-dialog">
        ...
        <form action="{{ route('kalibrasi.upload', $id) }}" method="post" enctype="multipart/form-data">
            {{ csrf_field() }}
            {{ method_field('PATCH') }}
                ...
                <div class="form-group">
                    ...
                    @if($upload_file != null)
                        ...
                        @else                            
                            ...
                            <img src="{{ asset('storage/TAM/BA/kalibrasi/'.date('Y',strtotime($created_at)).'/'.$upload_file)}}" style="width:45%; height:45%"><br>
...
<div class="modal fade" id="lihatFile{{$id}}" role="dialog" aria-labelledby="myModalLabel">                               
    <div class="modal-dialog modal-lg" width="100%">
        <div class="modal-content">
            ...
            <div class="modal-header text-center">
                <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                <h4 class="modal-title" id="myModalLabel">Detail Berita Acara Kalibrasi Mesin Nomor <strong>02.{{$nomor}}</strong></h4>
            </div>
            
            <div class="modal-body">
                <img src="{{ asset('storage/TAM/BA/kalibrasi/'.date('Y',strtotime($created_at)).'/'.$upload_file) }}" alt="{{$upload_file}}" width="100%">
